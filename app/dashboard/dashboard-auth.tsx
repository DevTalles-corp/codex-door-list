...
export function useOrganizerSession() {
  ...
  if (!session) throw new Error("useOrganizerSession must be used inside DashboardAuth");
  return session;
}
...
export default function DashboardAuth({ children }: { children: ReactNode }) {
    ...
    return (
      <main className="page-shell state-page">
        <div className="data-state">
          ...
          <p>El panel del organizador no está configurado en este momento.</p>
        </div>
      </main>
    );
  }
  if (loading) {
    return (
      <main className="page-shell state-page" aria-busy="true">
        <p className="visually-hidden" role="status">Cargando panel…</p>
        <div className="data-state" aria-hidden="true">
      ...
      <main className="page-shell auth-page">
        <section className="auth-card" aria-labelledby="auth-title">
          ...
          <form aria-busy={signingIn} onSubmit={signIn}>
            ...
            <button className="button button-primary" disabled={signingIn} type="submit">
